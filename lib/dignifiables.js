...
async function checkDignifiables(db, game, winnerId, guesses) {
    ...
    if (opponentCorrect.length === 0 && opponentGuesses.length > 0) {
    ...
    if (game.created_at && game.updated_at) {
        const duration = new Date(game.updated_at) - new Date(game.created_at);
    ...
    if (game.is_ai && game.ai_difficulty === 'hard') {
        const hardAIWins = await db.pool.query(
            `SELECT COUNT(*) as count FROM games 
             WHERE winner_id = $1 AND is_ai = TRUE AND ai_difficulty = 'hard'`,
            [winnerId]
        );
