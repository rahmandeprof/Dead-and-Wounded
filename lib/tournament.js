    ...
    return {
        ...
        async advanceRound(tournamentId) {
            ...
            if (nextRound > tournament.total_rounds) {
                throw new Error('Tournament already complete');
            }

            // Get all players for round-robin pairings
            const playersResult = await pool.query(
                'SELECT user_id as id, username FROM tournament_players WHERE tournament_id = $1',
                [tournamentId]
            );
            ...
            for (const pairing of rounds[nextRound - 1]) {
                await pool.query(
                    `INSERT INTO tournament_matches (tournament_id, round_number, player1_id, player2_id, status)
                     VALUES ($1, $2, $3, $4, 'pending')`,
                    [tournamentId, nextRound, pairing.player1_id, pairing.player2_id]
                );
            }
