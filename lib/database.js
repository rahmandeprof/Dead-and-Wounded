...
const gameOps = {
  ...
  async createAI(gameId, playerId, difficulty) {
    await pool.query(
      `INSERT INTO games(id, player1_id, is_ai, ai_difficulty, status, current_turn, created_at)
       VALUES($1, $2, true, $3, 'waiting', $2, NOW())`,
      [gameId, playerId, difficulty]
    );
    return { gameId };
  },

  async createAITimed(gameId, playerId, difficulty, timeControlSeconds) {
    await pool.query(
      `INSERT INTO games(id, player1_id, is_ai, ai_difficulty, status, current_turn, time_control_seconds, player1_time_remaining, created_at)
       VALUES($1, $2, true, $3, 'waiting', $2, $4, $4, NOW())`,
      [gameId, playerId, difficulty, timeControlSeconds]
    );
    return { gameId };
  },
...
const createSocialOps = require('./social');

// Export init function and operations
async function init() {
  return initDatabase();
}
...
module.exports = {
  ...
  socialOps: createSocialOps(pool),
