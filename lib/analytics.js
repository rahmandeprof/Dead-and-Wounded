    ...
    return {
        ...
        async getPlayerStats(userId) {
            ...
            const gameTypes = await pool.query(
                `SELECT 
                    COUNT(CASE WHEN is_ai = FALSE AND id NOT IN (SELECT game_id FROM tournament_matches WHERE game_id IS NOT NULL) THEN 1 END) as pvp_games,
                    COUNT(CASE WHEN is_ai = TRUE THEN 1 END) as ai_games,
                    COUNT(CASE WHEN id IN (SELECT game_id FROM tournament_matches WHERE game_id IS NOT NULL) THEN 1 END) as tournament_games
                 FROM games
        ...
        async getGameHistory(userId, filters = {}) {
            ...
            if (gameType === 'pvp') {
                query += ` AND g.is_ai = FALSE AND g.id NOT IN (SELECT game_id FROM tournament_matches WHERE game_id IS NOT NULL)`;
            } else if (gameType === 'ai') {
                query += ` AND g.is_ai = TRUE`;
            } else if (gameType === 'tournament') {
                query += ` AND g.id IN (SELECT game_id FROM tournament_matches WHERE game_id IS NOT NULL)`;
            }
